...
export function Iridescence({
  ...
  useEffect(() => {
    ...
    const render = () => {
      ...
      const r1 = Math.sin(time) * 10 + 226;
      const g1 = Math.cos(time * 0.8) * 10 + 232;
      const b1 = Math.sin(time * 1.2) * 5 + 240;
      
      const r2 = Math.cos(time * 1.1) * 15 + 224;
      const g2 = Math.sin(time * 0.9) * 15 + 242;
      const b2 = Math.cos(time * 1.3) * 5 + 254;
      
      const r3 = Math.sin(time * 0.7) * 10 + 203;
      const g3 = Math.cos(time * 1.4) * 10 + 213;
      const b3 = Math.sin(time * 0.8) * 5 + 225;
